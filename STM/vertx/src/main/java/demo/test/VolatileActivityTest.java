...
public class VolatileActivityTest extends ActivityBase {
    ...
    private void testVolatileActivityImpl(boolean abortApplicationTransaction)  {
        Container<TheatreService> containter = new Container<>(Container.TYPE.RECOVERABLE, Container.MODEL.EXCLUSIVE);

        // get a handle to an STM object for performing mandatory actions
        TheatreService mandatory = containter.create(new TheatreServiceImpl()); // TheatreService is Nested
