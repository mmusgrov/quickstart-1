...
public class JmsRecovery extends RecoverySetup {
    ...
    private static boolean inVM = false;
    ...
    public static void msgOp(String[] args) throws Exception {
        if (args[0].equals("-f")) {
            new JmsRecovery().failToSendGreeting();
        } else if (args[0].equals("-r")) {
            new JmsRecovery().recoverGreeting();
        } else if (args[0].equals("-s")) {
            new JmsRecovery().sendGreeting();
        } else if (args[0].equals("-a")) {
            new JmsRecovery().receiveGreeting();
        }
    }
    ...
    private void produceMessages(DummyXAResource.faultType fault, String ... messages) throws Exception {
    ...
    private int consumeMessages(int cnt, long millis) throws Exception {
        XAConnection connection = xacf.createXAConnection();
        int msgCnt = 0;
        ...
        try
        {
            ...
            // n.b. if we rolled back then since we consumed the messages inside a transaction they would still be available
            endJTATx(true);
    ...
    public void failToSendGreeting() throws Exception {
    ...
    public void recoverGreeting() throws Exception {
        runRecoveryScan();
    ...
    public void receiveGreeting() throws Exception {
        int cnt = consumeMessages(2, 500);
        if (cnt != 2)
            throw new RuntimeException("Expected 2 messages but received " + cnt);

        System.out.println("Message Count after receiving " + cnt);
    }
}
